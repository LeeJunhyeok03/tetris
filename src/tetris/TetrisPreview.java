...
if(block[0] != 8) { for(int i = 0; i < 4; i++) { g.setColor(colors[current.getType()]);
...
if(block[1] != 8) { for(int i = 0; i < 4; i++) { g.setColor(colors[current.getType()]); g.fill3DRect(margin/2 + w * (1+current.c[i]), margin/2 + w *
(3+current.r[i]), w, w, true);
repaint(); } } }
